...
export class VisitorChart extends React.PureComponent<{ series: VisitorSeries }> {

    render() {
        const labels = this.props.series.data.map((row) => formatLabel(row.date, this.props.series.interval));
        const chartData = {
            labels,
            datasets: [
                {
                    label: 'Visitors',
                    data: this.props.series.data.map((row) => row.uniqueVisitors),
                    borderColor: '#2563eb',
                ...
                {
                    label: 'Pageviews',
                    data: this.props.series.data.map((row) => row.pageviews),
                    borderColor: '#a1a1aa',
        ...
        return (
            <div className="rounded-lg border border-zinc-200 bg-white p-3">
                ...
                <div className="mt-2 h-56">
                    { this.props.series.data.length === 0 ? (
